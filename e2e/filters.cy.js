  ...
  describe('on citeproc file', () => {
    ...
    it('should display filter for each link type', () => {
      cy.get('@linkFiltersContainer')
        .find('.filter input')
        .should('have.length', linkFilters.length);
    ...
    it('should hide links on click on link type', () => {
      ...
      cy.get('@inputs').last().click();
      cy.get('[data-link]:visible').should('have.length', 2);
    });

    describe('with alt key for links', () => {
      function assertLinkFiltersAreChecked(names) {
        cy.get('@linkFiltersContainer')
          .find('.filter input:checked')
          .should('have.length', names.length)
          .each((elt) => {
      ...
      it('should uncheck all link inputs but not clicked one if alt key is pressed', () => {
        ...
        assertLinkFiltersAreChecked(['adjacent-concept']);
      });
